...
export default function Register(){
  ...
  const submit = async (e)=>{
    ...
    try {
      const data = await fetcher('/auth/register', { method: 'POST', body: form, auth:false });
      setToken(data.token);
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 flex items-center justify-center min-h-screen">
        <div className="max-w-md w-full p-8 bg-white/95 backdrop-blur-sm rounded-xl shadow-2xl fade-in border border-white/20">
          <h1 className="text-3xl font-bold mb-6 text-indigo-900 text-center drop-shadow-md">Create Account</h1>
          <p className="text-gray-700 mb-8 text-center drop-shadow-sm">Join us and start managing your data</p>
          <form onSubmit={submit} className="space-y-4">
            ...
            <button type="submit" className="btn-primary w-full hover-scale shadow-lg">Sign Up</button>
          </form>
          <div className="mt-6 text-center">
            <Link href="/login" className="text-indigo-600 hover:text-indigo-500 font-medium">Already have an account? Sign In</Link>
          </div>
        </div>
      </div>
    </div>
